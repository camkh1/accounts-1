...
<html>
	...
	<body class="login">
		...
		<div class="box">
		@if (Session::has ( 'user' ))
			@foreach(Session::get ( 'user' ) as $error)
			<div class="alert alert-danger">{{trans('login.'.$error[0])}}</div>
			@endforeach
		@endif
		@if(Input::get('error'))
		<div class="alert alert-danger">{{Input::get('error')}}</div>
		@endif
		  <div class="content">
		    <form class="form-vertical login-form" action="{{URL::to('member')}}" method="post">
		      ...
		      <div class="form-actions">
		        <label class="checkbox pull-left">
		          ...
		          Remember me</label>
		        <button id="submittbn" type="submit" name="btnLogin" value="Login" class="submit btn btn-primary pull-right"> Sign In <i class="icon-angle-right"></i> </button>
		      </div>
		      <input type="hidden" name="continue" value="{{Input::get('continue')}}"/>
		    </form>

		    <form class="form-vertical register-form" action="{{URL::to('register')}}" method="post" style="display:none">
		  ...
		  <div class="inner-box">
		    <div class="content"> <i class="icon-remove close hide-default"></i> <a href="#" class="forgot-password-link">Forgot Password?</a>
		      <form class="form-vertical forgot-password-form hide-default" action="{{URL::to('admin/send-forget-password')}}" method="post">
		        <div class="form-group">
