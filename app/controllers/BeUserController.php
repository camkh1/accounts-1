...
class BeUserController extends BaseController {
	 ...
	 * get divice function
	 */
	public function myAccountDetail($userID, $type=null,$currentIP=null) {
		header('Access-Control-Allow-Origin: *');
		Session::forget ( 'user' );
		$users = $this->user->where('id','=',$userID)->first();
		/*check status user*/
		$user = Auth::user();
		$online = $users->u_online;
		$uip = $users->u_ip;
		if(!empty($currentIP) && $currentIP != $uip && $online == 1) {
			Session::push ( 'user.u_ip', 'Your_used_with_other');
		}

		$this->user->where('id','=',$userID)->update(array('lastActiveTime'=> time(), 'u_online'=>1));

		if(Session::has ( 'user' )) {
			$data = array('error'=> trans('login.Your_used_with_other'));
			if($type == 'local') {
	...
	/* check user online status by cron job */
	...
	/* end check user online status by cron job */
}
